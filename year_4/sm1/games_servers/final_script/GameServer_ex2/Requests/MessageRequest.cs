        ...
        public static void Get(IWebSocketSession session, string data)
        {
            ...
            try
            {
                ...
                if (curr_user != null)
                {
                    ...
                    Dictionary<string, object> msg_data = JsonConvert.DeserializeObject<Dictionary<string, object>>(data);
                    if (msg_data.ContainsKey("Service"))
                    {
                        string service = msg_data["Service"].ToString();
                        switch (service)
                        {
                            case "ReadyToPlay":
                                response = ReadyToPlayRequest.Get(curr_user, msg_data);
                                break;
                            case "SendMove":
                                //TODO: send move
                                break;
